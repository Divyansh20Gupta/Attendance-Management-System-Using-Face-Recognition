...
# ---------------------- Project Modules (safe import) ----------------------
# If any of these modules raise ImportError, show a warning but allow UI to start.
missing_modules = []
try:
    import show_attendance
except Exception as e:
    show_attendance = None
    missing_modules.append(("show_attendance", e))
try:
    import takeImage
except Exception as e:
    takeImage = None
    missing_modules.append(("takeImage", e))
try:
    import trainImage
except Exception as e:
    trainImage = None
    missing_modules.append(("trainImage", e))
try:
    import automaticAttedance
except Exception as e:
    automaticAttedance = None
    missing_modules.append(("automaticAttedance", e))
...
def load_image_safe(path, size=None):
    if not os.path.exists(path):
        # print a console warning and return None
...
# ---------------------- Helper: run long task in background ----------------------
def run_in_thread(fn, *args, daemon=True):
    t = threading.Thread(target=lambda: _thread_wrapper(fn, *args), daemon=daemon)
    t.start()
    return t

def _thread_wrapper(fn, *args):
    try:
        fn(*args)
    except Exception:
        # print full traceback so developer can debug
        traceback.print_exc()
...
def open_register():
    ...
    def take_img_action():
        ...
        if not enrollment or not name:
            messagebox.showwarning("Input required", "Please enter Enrollment No and Name.")
            return
        if takeImage is None:
            messagebox.showerror("Module missing", "takeImage module is not available.")
            return
        # run in background so UI stays responsive
        run_in_thread(takeImage.TakeImage, enrollment, name, HAAR_PATH,
                      TRAIN_IMAGE_DIR, note_lbl, None, text_to_speech)

        # clear fields after starting capture (optional)
    ...
    def train_img_action():
        if trainImage is None:
            messagebox.showerror("Module missing", "trainImage module is not available.")
            return
        run_in_thread(trainImage.TrainImage, HAAR_PATH, TRAIN_IMAGE_DIR,
...
def start_attendance():
    if automaticAttedance is None:
        messagebox.showerror("Module missing", "automaticAttedance module is not available.")
        return
    # run attendance workflow in background
    run_in_thread(automaticAttedance.subjectChoose, text_to_speech)

def view_attendance():
    if show_attendance is None:
        messagebox.showerror("Module missing", "show_attendance module is not available.")
        return
    run_in_thread(show_attendance.subjectchoose, text_to_speech)
...
# Warn user if modules failed to import
if missing_modules:
    msg = "Some optional modules failed to import:\n"
    for name, err in missing_modules:
        msg += f"- {name}: {err.__class__.__name__}\n"
    print(msg)  # console
    # show an unobtrusive popup once the UI has loaded
    def show_import_warning():
        messagebox.showwarning("Module Import Warning", msg)
    root.after(1000, show_import_warning)
